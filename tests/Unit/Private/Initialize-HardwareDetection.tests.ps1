...
Describe 'Initialize-HardwareDetection' -Skip:(-not $builtModule) {
    Context 'Function existence' {
        It 'Should exist as a private function in the module' {
            InModuleScope $script:moduleName {
                Get-Command -Name 'Initialize-HardwareDetection' -ErrorAction SilentlyContinue | Should -Not -BeNullOrEmpty
            }
        }
    }
}
