...
Describe 'Invoke-MitigationAssessment' -Skip:(-not $builtModule) {
    Context 'Function existence' {
        It 'Should exist as a private function in the module' {
            InModuleScope $script:moduleName {
                Get-Command -Name 'Invoke-MitigationAssessment' -ErrorAction SilentlyContinue | Should -Not -BeNullOrEmpty
            }
        }
    }
}
